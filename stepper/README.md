...
This repository contains the source code for The Hacker Fab's open source stepper software. It provides a desktop GUI for pattern selection, stage alignment, timed UV exposure, and live camera preview.
...
- Internet connection for the first run (downloads ~1–2 GB of dependencies)

No Python installation needed — the run scripts install everything automatically.
...
If PowerShell blocks the script, run this once in a PowerShell window:
```
...
1. The [uv](https://docs.astral.sh/uv/) package manager is installed automatically (if not already present).
2. Python 3.12 and all dependencies are downloaded into an isolated environment (`.venv/`).  
   **This takes a few minutes** — PyTorch and the YOLO model are large.
3. The startup dialog appears to select a config file and connect to the GRBL stage.

Subsequent runs start in seconds.

### Basler camera users

`pypylon` is an optional dependency.  Run the following once to enable it:

```bash
uv sync --extra basler
```
